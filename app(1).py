...
def real_time_developer(state: CrewState):
    ...
    response = ll_flash.invoke(prompt)
...
def manager_decision_node(state: CrewState):
    ...
    return {"next_step": "archiver" if command == "store" else "exit"}
...
workflow.add_conditional_edges("task_input", route_from_input)
workflow.add_edge("developer", "tester")
workflow.add_edge("tester", "manager_decision")


def route_from_decision(state: CrewState):
    return "archiver" if state.get("next_step") == "archiver" else END


workflow.add_conditional_edges("manager_decision", route_from_decision)
...
class AgentRequest(BaseModel):
    ...
    command: str = Field("store", description="store or another")
...
def run_agent(request: AgentRequest):
    ...
    if command not in {"store", "another"}:
        raise HTTPException(
            status_code=400,
            detail="command must be 'store' or 'another'",
        )
